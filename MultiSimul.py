...
for func in FuncGrowth:
    for ratio in ProdDegRatio:
        ...
        while t <= tmax:
            ...
            l0i[tIndex, :] = growth(fibersStress, l0i[tIndex-1], dt, func) # (M) we compute the new "original" lengths of each fiber and store it in the corresponding matrix


        outputMatrixL.append(l)
        outputMatrixK.append(k)
        legend.append(str(func) + "-" + str(ratio))
